...
SELECT pg_size_pretty(pg_total_relation_size(observations_pkey)) as idx_1_size,
    pg_size_pretty(pg_total_relation_size(observations_location_id_datetime_idx)) as idx_2_size;

  SELECT 
    pg_size_pretty(pg_table_size(sensors.observations)) as tbl_size, 
    pg_size_pretty(pg_indexes_size(observations_location_id_datetime_idx)) as idx_size,
